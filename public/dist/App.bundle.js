...
var clearFilters = function clearFilters(reset) {
  if (!reset) return;
...
var resetDate = function resetDate() {
  var start = (0, _bling.$)("#startDatetime");
  var end = (0, _bling.$)("#endDatetime");
  start.value = new Date().toISOString().slice(0, 10);
  end.value = "";
};

var resetPrice = function resetPrice() {
  var min = (0, _bling.$)("#minPrice");
  var max = (0, _bling.$)("#maxPrice");
  min.value = "0.00";
  max.value = "1000.00";
};
...
function renderMarkers(places) {
  ...
  var groupedPlaces = places.reduce(function (acc, place) {
    var key = place.location.coordinates;
    (acc[key] ? acc[key] : acc[key] = null || []).push(place);
    return acc;
  }, {});

  // Convert to an array
  groupedPlaces = Object.keys(groupedPlaces).map(function (key) {
    return groupedPlaces[key];
  });
  ...
  markers = (0, _helpers.clearOverlays)(markers);
  markers = groupedPlaces.map(function (place) {
    var _place$0$location$coo = _slicedToArray(place[0].location.coordinates, 2),
        placeLng = _place$0$location$coo[0],
        placeLat = _place$0$location$coo[1];
    ...
    var marker = new google.maps.Marker({ map: map, position: position, label: label });
    marker.place = place;
    return marker;
  });
    ...
    return marker.addListener("click", function () {
      ...
      for (var i = 0; i < this.place.length; i++) {
        ...
        if (this.place[i].price_range.max_price) {
          price = "\xA3" + this.place[i].price_range.min_price.toFixed(2) + " to \xA3" + this.place[i].price_range.max_price.toFixed(2);
        }
        if (this.place[i].price_range.min_price === 0.0 || this.place[i].price_range.min_price === 0) {
          price = "Free to \xA3" + this.place[i].price_range.max_price.toFixed(2);
        }

        html += "\n        <div class=\"popup__event" + (i > 0 ? " hide" : "") + "\">\n            <div class=\"event\">\n            <img class=\"event__img\" src=\"" + image + "\" alt=\"\">\n            <h3 class=\"event__title\"><a href=\"" + this.place[i].website + "\">" + this.place[i].name + "</a></h3>\n            <dl class=\"event__details\">\n              <dt class=\"offscreen\">Organiser:</dt>\n              <dd class=\"event__item event__item--organiser\">" + this.place[i].organisation + "</dd>\n              <dt class=\"offscreen\">Address:</dt>\n              <dd class=\"event__item\"><span class=\"event__icon\"><svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 20 20\"><path d=\"M10 20S3 10.87 3 7a7 7 0 1 1 14 0c0 3.87-7 13-7 13zm0-11a2 2 0 1 0 0-4 2 2 0 0 0 0 4z\"></path></svg></span>" + this.place[i].location.address + "</dd>\n              <dt class=\"offscreen\">Event date:</dt>\n              <dd class=\"event__item\"><span class=\"event__icon\"><svg xmlns=\"http://www.w3.org/2000/svg\" fill=\"#f5faff\" viewBox=\"0 0 20 20\"><path d=\"M1 4c0-1.1.9-2 2-2h14a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V4zm2 2v12h14V6H3zm2-6h2v2H5V0zm8 0h2v2h-2V0zM5 9h2v2H5V9zm0 4h2v2H5v-2zm4-4h2v2H9V9zm0 4h2v2H9v-2zm4-4h2v2h-2V9zm0 4h2v2h-2v-2z\"></path></svg></span>" + this.place[i].display_date + "</dd>\n              " + (price ? '<dt class="offscreen">Price:</dt><dd class="event__item"><span class="event__icon"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M20 12v5H0v-5a2 2 0 1 0 0-4V3h20v5a2 2 0 1 0 0 4zM3 5v10h14V5H3zm7 7.08l-2.92 2.04L8.1 10.7 5.27 8.56l3.56-.08L10 5.12l1.17 3.36 3.56.08-2.84 2.15 1.03 3.4L10 12.09z"></path></svg></span>${price}</dd>' : "") + "\n            </dl>\n          </div>\n          </div>";
      }
...
function togglePadlock(padlock) {
  ...
  padlock.addEventListener("click", function () {
    var closed = this.querySelector(".padlock-img--closed");
    var label = this.querySelector(".padlock-label");

    closed.style.display = closed.style.display === "none" ? "block" : "none";
    label.title = label.title === "Click to disable button." ? "Click to enable button." : "Click to disable button.";
  });
}
...
exports.clearOverlays = function (markers) {
...
var getPriceRange = function getPriceRange(min, max) {
  if (!min && !max) return "";
...
// Filter places
exports.filterPlaces = function (places) {
  ...
  var free = (0, _bling.$)("#free");
  var searchTerm = (0, _bling.$)("#organisationSearch").value;

  if (searchTerm) {
    places = places.filter(function (place) {
      return (place.name + " " + place.organisation).toLowerCase().indexOf(searchTerm.toLowerCase()) >= 0;
    });
  }

  if (lat && lng) {
    places = places.filter(function (place) {
      return place.location.coordinates[1] === lat && place.location.coordinates[0] === lng;
    });
  }

  if (free.checked) {
    places = places.filter(function (place) {
      return place.is_free;
    });
  }
...
var _clearFilters = __webpack_require__(12);
